...
class Engine
{
	...
	void reset() { this->_board = bitboard::BBoards::EMPTY; }

	/// set board to `board`, return false if `board` illegal
	bool set(std::string board) { return board == ""; }
	...
	/// Return actual board
	std::string board() { return ""; }

	/// Play best move for `player`, return new board
	std::string play_best(std::string player) { return player; }
